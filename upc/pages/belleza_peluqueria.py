...
class BellezaPeluqueria(rx.State):
...
@rx.page(route="/belleza_peluqueria", title="Belleza y Peluquería", on_load=State.get_users_belleza_peluqueria)
@template
def belleza_peluqueria() -> rx.Component:
    return rx.hstack(
        rx.heading("Belleza y Peluquería"),
        table(State.users),
        direction="column",
    )
